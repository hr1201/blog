...
具体来说，当组件的状态发生**改变**时，Vue.js会**先生成新的虚拟节点树**，然后**通过Diff算法**比较新旧虚拟节点树的差异，并生成一棵**补丁（Patch）树**。补丁树中的每个节点表示一次DOM操作，例如添加、删除、修改元素属性等。最后，Vue.js会根据补丁树对真实DOM进行更新。


之所以需要虚拟dom，是因为一个元素含有许多的属性（如下图），直接操作DOM的话性能会偏低，这时候我们就可以用**js的计算性能**(也就是通过diff算法)来换取操作DOM所消耗的性能，操作js是非常快的。
